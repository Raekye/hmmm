...
void Lexer::generate() {
	...
	for (size_t i = 0; i < this->rules.size(); i++) {
		regex_nfa_generator.new_rule(this->rules.at(i));
		this->rules_regex.at(i)->accept(&regex_nfa_generator);
	}
...
void Lexer::debug() {
	...
	for (std::unique_ptr<RegexDFAState> const& state : this->dfa->states) {
